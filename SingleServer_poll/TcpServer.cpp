...
void TcpServer::start() {
    ...
    while (true) {
        // 等待活动（阻塞，无超时）
        int activity = poll(pollfds.data(), pollfds.size(), -1); // 不设置超时
        if (activity < 0 && errno != EINTR) { // 用于判断系统调用或库函数的错误是否不是由 “信号中断” 引起的
        ...
        if (pollfds[0].revents & POLLIN) {
            ...
            client_pollfd.events = POLLIN;  // 关注读事件（客户端发数据）
            client_pollfd.revents = 0;
            pollfds.push_back(client_pollfd);
        }

        // 处理客户端数据（遍历所有客户端socket）
        for (auto it = pollfds.begin()+1; it != pollfds.end(); ) {
            if (it->revents & POLLIN) { //这里必须要用位运算，自己体会下，例如 010==110就是false了
                int client_socket = it->fd;
                // 创建子进程处理请求
                pid_t pid = fork();
                if (pid < 0) {
                    std::cerr << "Error: fork failed" << std::endl;
                    close(client_socket);
                    it = pollfds.erase(it);  // 移除无效连接
                }
                else if (pid == 0) {
                    // 子进程：处理客户端请求
